...
mQH <- occu(~ qualite ~ Humide + annee, data = licat.data)
mQE <- occu(~ qualite ~ Eau + annee, data = licat.data)
mQR <- occu(~ qualite ~ Route + annee, data = licat.data)
mQA <- occu(~ qualite ~ Agriculture + annee, data = licat.data)
mQS <- occu(~ qualite ~ Site + annee, data = licat.data)

##Détectabilité varie selon le type de perturbation et occupation varie selon ...
mPH <- occu(~ disturbance ~ Humide + annee, data = licat.data)
mPE <- occu(~ disturbance ~ Eau + annee, data = licat.data)
mPR <- occu(~ disturbance ~ Route + annee, data = licat.data)
mPA <- occu(~ disturbance ~ Agriculture + annee, data = licat.data)
mPS <- occu(~ disturbance ~ Site + annee, data = licat.data)
...
mDH <- occu(~ date ~ Humide + annee, data = licat.data)
mDE <- occu(~ date ~ Eau + annee, data = licat.data)
mDR <- occu(~ date ~ Route + annee, data = licat.data)
mDA <- occu(~ date ~ Agriculture + annee, data = licat.data)
mDS <- occu(~ date ~ Site + annee, data = licat.data)
```
...
predData <- data.frame(annee = c(0,0),
Eau = c(0, 1),
Humide = c(0, 0),
Route = c(0, 0),
Agriculture = c(0, 0),
...
```
Je ne comprend pas comment utiliser les prédictions. Je ne sais pas si je fais mal mon tableau predData??Je pense aussi que je vais avoir besoin de ça pour créer des figures?

```{r pred detect}
extractX(cand.set = Cands, modnames = Model.names, parm.type = "detect")
predData <- data.frame(annee = c(0,0),
qualite = factor("qualitegood", 
                 levels = levels()),
Humide = c(0, 0),
Route = c(0, 0),
Agriculture = c(0, 0),
